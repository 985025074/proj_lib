...
1. “TCP window full” 的核心含义
当 Wireshark 标记 “TCP window full” 时，本质是捕获到了以下场景：

发送端视角：发送端刚刚发送的一段数据，其大小等于或接近接收端上一次通过 ACK 告知的 “剩余接收窗口大小”，导致接收端的窗口被 “填满”（剩余窗口变为 0 或极小）。
接收端视角：接收端的 TCP 缓冲区已被当前数据占满，暂时没有空闲空间接收新数据，因此下一次发送 ACK 时，会将 “Window Size” 设为 0（即 “零窗口”），并触发 “TCP Zero Window” 标记。
